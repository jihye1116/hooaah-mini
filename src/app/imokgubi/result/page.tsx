...
export default function ImokgubiResultPage({ searchParams }: ResultPageProps) {
  ...
  useEffect(() => {
    searchParams.then((resolvedParams) => {
      ...
      if (resolvedParams.resultText) {
        try {
          const parsedResult = JSON.parse(resolvedParams.resultText);
          setResult(parsedResult);
        } catch (e) {
          setErrorText(resolvedParams.resultText);
        }
      }

      if (resolvedParams.resultImage) {
        setResultImage(resolvedParams.resultImage);
      }
    });
  }, [searchParams]);
    ...
    <div className="min-h-screen w-full bg-gray-50 pb-32">
      ...
      <div className="fixed bottom-0 w-full max-w-[480px] bg-white p-4 pb-8">
        <div className="flex gap-3">
          <button
            onClick={() => router.back()}
            className="flex-1 rounded-xl bg-gray-300 py-4 font-bold text-white"
          >
            다시하기
          </button>
        </div>
